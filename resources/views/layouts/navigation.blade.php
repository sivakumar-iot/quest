...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    {{-- Public Test Link --}}
                    <x-nav-link :href="route('test.entry')" :active="request()->routeIs('test.entry')">
                        {{ __('Take Test') }}
                    </x-nav-link>
                </div>
            </div>
                ...
                @else {{-- User is a guest --}}
                    <div class="space-x-4">
                        ...
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                {{ __('Register') }}
                            </a>
                        @endif
                    </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            {{-- Admin Links (Conditional) for Responsive Menu --}}
            @auth
                @if (Auth::user()->isAdmin())
            ...
            {{-- Public Test Link for Responsive Menu --}}
            <x-responsive-nav-link :href="route('test.entry')" :active="request()->routeIs('test.entry')">
                {{ __('Take Test') }}
            </x-responsive-nav-link>
        </div>
