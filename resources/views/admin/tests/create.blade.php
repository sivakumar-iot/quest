...
<x-app-layout>
                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('tests.store') }}">
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="test_code" class="block mt-1 w-full" type="text" name="test_code" :value="old('test_code')" required />
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="duration_minutes" class="block mt-1 w-full" type="number" name="duration_minutes" :value="old('duration_minutes')" required min="1" />
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="instructions" name="instructions" rows="3" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('instructions') }}</textarea>
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="pass_percentage" class="block mt-1 w-full" type="number" name="pass_percentage" :value="old('pass_percentage')" min="0" max="100" />
                        ...
                        <div id="test_rules_container">
                            {{-- Initial rule block (or loop through old values if validation fails) --}}
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Load existing rules from old input (e.g., after validation failure)
            @if (old('rules'))
                ruleIndex = 0; // Reset index for old input
                @foreach (old('rules') as $index => $rule)
                    addRuleBlock({
                        module_id: "{{ $rule['module_id'] ?? '' }}",
                        topic_id: "{{ $rule['topic_id'] ?? '' }}",
                        question_type: "{{ $rule['question_type'] ?? '' }}",
                        number_of_questions: "{{ $rule['number_of_questions'] ?? 1 }}"
                    });
                @endforeach
            @else
                // If no old rules, and no existing ones (for edit form), add one empty block
